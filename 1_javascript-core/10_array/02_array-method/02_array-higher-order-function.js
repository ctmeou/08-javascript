...
// 기본적으로는 문자열 정렬로 한 자리 수나 세 자리 수가 올바르지 않게 정렬되는 모습을 보임
// => 다른 정렬 기준을 사용하려면 compare 함수를 인수로 전달해야 함
numbers.sort();

console.log(`정렬 후 numbers : ${ numbers }`);

// 숫자 오름차순 정렬에 사용될 function
...
// Array.prototype.forEach : for를 대체할 수 있는 고차 함수
...
// Array.prototype.reduce : 배열을 순회하며 각 요소에 대하여 이전의 콜백 함수 실행 반환 값을 전달하여 콜백 함수를 실행하고 그 결과를 반환
...
// Array.prototype.findIndex : 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백 함수를 실행하여 그 결과가 참인 첫 번째의 인덱스를 반환
