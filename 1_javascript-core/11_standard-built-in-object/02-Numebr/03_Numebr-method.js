...
// Number.isFinite : 인수로 전달된 숫자 값이 정상적인 유한수인지 검사하여 결과를 불리언으로 반환한다.
...
// Number.isInteger : 인수로 전달된 숫자 값이 정수인지 검사하여 결과를 불리언으로 반환한다.
...
// Number.isNaN : 인수로 전달된 숫자 값이 NaN인지 검사하여 결과를 불리언으로 반환한다.
console.log(Number.isNaN(NaN));                                     // true
console.log(Number.isNaN(undefined));                               // false
console.log(isNaN(undefined));                                      // 빌트인 전역함수 isNaN은 암묵적 타입변환을 한다.
console.log('----------------------');

// Number.isSafeInteger : 인수로 전달된 숫자 값이 안전한 정수인지 검사하여 결과를 불리언으로 반환한다.
...
// 반올림 하는 소수점 이하 자릿수를 나타내는 0 ~ 20 사이의 정수값을 인수로 전달할 수 있다.
console.log((1.23456).toFixed());                                   // 1
console.log((1.23456).toFixed(3));                                  // 1.235
console.log((1.23456).toFixed(1));                                  // 1.2
console.log('----------------------');

// Number.prototype.toPrecision : 인수로 전달 받은 전체 자릿수까지 유효하도록 나머지 자릿수를 반올림하여 문자열로 반환한다.
// 0 ~ 21사이의 정수 값을 인수로 전달할 수 있으며 생략하면 기본 값 0이 지정된다.
